...
// Import the new registry system for extensible matcher handling
var DocumentMatcherRegistry = require("./styles/document-matcher-registry").DocumentMatcherRegistry;
var defaultRegistry = require("./styles/document-matcher-registry").defaultRegistry;
var MatcherErrorHandler = require("./styles/matcher-error-handler");

// Initialize the error handler
var errorHandler = new MatcherErrorHandler();
...
exports.configureMatchers = configureMatchers;
...
function documentMatcherRule() {
    ...
    var breakMatcher = sequence(
        ...
        // First, try the traditional built-in break types
        switch (breakType) {
        case "line":
        ...
        default:
            // Use the registry system to handle unknown break types
            return handleUnknownMatcher(breakType, "break", {breakType: breakType});
        }
    });
...
/**
 * Handle unknown matcher types using the registry system
 * This replaces the TODO comment at the original line 198
 */
function handleUnknownMatcher(matcherType, category, options) {
    options = options || {};
    
    // Create context for error handling
    var context = {
        availableTypes: defaultRegistry.getAvailableTypes(),
        category: category,
        parseContext: "style-reader"
    };
    
    // Try to create matcher using the registry
    var result = defaultRegistry.createMatcher(matcherType, options);
    
    if (result.value) {
        // Registry successfully created a matcher
        return result.value;
    }
    
    // Registry couldn't create matcher - use error handler for better feedback
    var errorResult = errorHandler.handleUnknownType(matcherType, context);
    
    if (errorResult.value) {
        // Error handler provided a recovery matcher
        return errorResult.value;
    }
    
    // Fallback: return null to indicate no matcher could be created
    // The calling code will need to handle this appropriately
    return null;
}

/**
 * Configure matchers using configuration object or file
 */
function configureMatchers(config) {
    if (typeof config === "string") {
        // Assume it's a file path
        try {
            var MatcherConfiguration = require("./styles/matcher-configuration");
            var configuration = new MatcherConfiguration();
            configuration.loadConfiguration(config);
            
            // Apply configuration to registry
            defaultRegistry.loadConfiguration(configuration.getConfiguration());
            
            return results.success("Configuration loaded successfully");
        } catch (error) {
            return new results.Result(null, [results.error(error)]);
        }
    } else if (typeof config === "object") {
        try {
            // Apply configuration directly to registry
            defaultRegistry.loadConfiguration(config);
            return results.success("Configuration applied successfully");
        } catch (error) {
            return new results.Result(null, [results.error(error)]);
        }
    }
    
    return new results.Result(null, [results.error(new Error("Invalid configuration format"))]);
}

/**
 * Register a plugin with the matcher registry
 */
function registerPlugin(plugin) {
    try {
        defaultRegistry.registerPlugin(plugin);
        return results.success("Plugin registered successfully");
    } catch (error) {
        return errorHandler.handlePluginError(plugin.name || "Unknown", error, {
            source: "style-reader"
        });
    }
}
...
/**
 * Enhanced document matcher rule that supports extensible matching
 * This extends the original documentMatcherRule to handle custom matchers
 */
function createExtendedMatcher(matcherType, options) {
    // First try the registry system
    var result = defaultRegistry.createMatcher(matcherType, options);
    ...
    // If registry fails, log warnings but continue parsing
    if (result.messages && result.messages.length > 0) {
        result.messages.forEach(function(message) {
            if (message.type === "warning") {
                console.warn("Style Reader Warning:", message.message);
            } else if (message.type === "error") {
                console.error("Style Reader Error:", message.message);
            }
        });
    }
    
    // Return null to indicate no matcher was created
    return null;
}
